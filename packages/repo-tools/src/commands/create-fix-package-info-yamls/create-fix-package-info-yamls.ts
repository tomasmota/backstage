  ...
  const results = await Promise.allSettled<void>(
    packages.map(({ packageJson, dir }) =>
      limit(async () => {
        ...
        await readFile(infoYamlPath, { encoding: 'utf-8' })
          .then(yamlString =>
            fixCatalogInfoYaml({
...
function fixCatalogInfoYaml(options: FixOptions) {
  ...
  const badType = yamlJson.spec?.type !== 'backstage-package';
  const badRole = yamlJson.spec?.backstageRole !== packageJson.backstage.role;
  const badDesc = yamlJson.metadata.description !== packageJson.description;

  if (badOwner || badTitle || badName || badType || badRole || badDesc) {
...
function createOrMergeEntity(
  ...
  return {
    ...
    spec: {
      ...
      ...existingEntity.spec,
      type: 'backstage-package',
      backstageRole: packageJson.backstage.role,
      owner,
    },
  };
}
